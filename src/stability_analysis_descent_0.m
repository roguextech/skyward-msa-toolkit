...
% linearization around the drogue initial conditions
...
% Point around which linearization is evaluated: descent initial conditions
...
X0 = [Ya(end,1:13)'; posPara0'; velPara0'; Ya(end, 18:20)'];  
t = Ta(end);                                            %time instant in which the linearization is evaluated
...
S_para = drg.S;                                                   % [m^2]   Surface
CD_para = drg.CD;
D0 = sqrt(4*drg.S/pi);
t0 = drg.nf * D0/V_norm_para;
tx = t0 * drg.CX^(1/drg.m);
SCD0 = S_para*CD_para;                                            % [m^2]   Surface
t0p(1) = Ta(end);
...
%     SCD_para = 0;
% elseif dt < tx
    SCD_para = SCD0 * (dt/t0)^drg.m;
% else
%     SCD_para = SCD0 * (1+(drg.CX-1)*exp(-2*(dt-tx)/t0));
% end
