    ...
    for j = 1:N2
        ...
        for s = 1:Ns
            ...
            for k = 1:2
                if print
                    clc
                    perc = round((i-1)/N1*100 + (j-1)/N2*(100/N1) + (s-1)/Ns*(100/N1/N2) + (k-1)/2*(100/N1/N2/Ns));
                ...
                data{i, j, s}.(mass_condition{k}).Coeffs = Coeffs;
                data{i, j, s}.(mass_condition{k}).State = State;
                if k == 1
                    data{i, j, s}.c_max = C1;
                    data{i, j, s}.c_min = C2;
                    data{i, j, s}.h = H;
                    data{i, j, s}.shape = shape(s);
                end
            end
        end
